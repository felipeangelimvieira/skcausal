...
def test_basic_prediction(linear_data):
    X, y = linear_data
    llr = LocalLinearRegression(bandwidth=10.0)
    llr.fit(X, y)
...
def test_constant_prediction(constant_data):
    X, y = constant_data
    llr = LocalLinearRegression(bandwidth=0.1)
    llr.fit(X, y)
    preds = llr.predict([[0.5], [1.5]])
...
def test_zero_weights_behavior(constant_data):
    X, y = constant_data
    llr = LocalLinearRegression(bandwidth=0.01, kernel="epanechnikov")
    llr.fit(X, y)
    preds = llr.predict([[10.0]])  # Far from training data
    assert np.isclose(preds[0], 5.0)


def test_large_bandwidth_equivalence():
    X, y = make_regression(n_samples=100, n_features=2, noise=0, random_state=42)

    # Our model with large bandwidth
    llr = LocalLinearRegression(bandwidth=1e6)
    llr.fit(X, y)
...
def test_multi_feature_support(multi_feature_data):
    ...
    llr = LocalLinearRegression(bandwidth=1.0)
    llr.fit(X, y)
...
def test_epanechnikov_kernel():
    X = np.array([[0], [1], [2]])
    y = np.array([0, 1, 2])
    llr = LocalLinearRegression(bandwidth=1.0, kernel="epanechnikov")
    llr.fit(X, y)
...
def test_parameter_persistence():
    llr = LocalLinearRegression(bandwidth=2.5, kernel="epanechnikov")
    assert llr.bandwidth == 2.5
