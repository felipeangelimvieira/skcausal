...
::: {.callout-note}
This keeps the categorical example aligned with the continuous-treatment API: define a treatment table, call `predict`, and then visualize the returned response values.
:::
...
_ = direct.fit(X, t, y)
direct_curve = np.asarray(direct.predict(t=levels), dtype=float).reshape(-1)
```
...
_ = ipw.fit(X, t, y)
ipw_curve = np.asarray(ipw.predict(t=levels), dtype=float).reshape(-1)
```
...
_ = dr.fit(X, t, y)
dr_curve = np.asarray(dr.predict(t=levels), dtype=float).reshape(-1)
```
