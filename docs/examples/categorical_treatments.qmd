...
## What changes from binary treatments
...
There are, as of now, three estimators designed for categorical treatments in particular:
...
`predict` now accepts the requested treatment table as its first argument and returns one average response per row in `levels`. If you omit `X`, X-aware estimators reuse the covariate sample stored during `fit`. `predict_curve` remains a convenience alias with the same semantics.

::: {.callout-note}
This keeps the categorical example aligned with the continuous-treatment API: define a treatment table, call `predict` or `predict_curve`, and then visualize the returned response values.
:::
...
_ = direct.fit(X, t, y)
direct_curve = np.asarray(direct.predict_curve(t=levels), dtype=float).reshape(-1)
```
...
_ = ipw.fit(X, t, y)
ipw_curve = np.asarray(ipw.predict_curve(t=levels), dtype=float).reshape(-1)
```
...
_ = dr.fit(X, t, y)
dr_curve = np.asarray(dr.predict_curve(t=levels), dtype=float).reshape(-1)
```
...
comparison = (
    ...
    .with_columns(
        ...
        pl.Series("direct", direct_curve),
        pl.Series("ipw", ipw_curve),
        pl.Series("dr", dr_curve),
    )
)
...
The package also exposes general-purpose estimators that can work with categorical treatments here and with continuous treatments in other workflows: `DirectNoCovariates`, `DirectRegressor`, `DoublyRobustPseudoOutcome`, and `GPS`. We do not benchmark `Pipeline` here because it is a wrapper for composing preprocessing with one of those estimators rather than a distinct estimation strategy.

For this categorical example, the nuisance models still need to match the observed treatment type, so we keep using a categorical density estimator inside the broader estimators below. `GPS` now passes the raw treatment column into its outcome model, so the preprocessing pipeline explicitly handles the categorical encoding, expands the `gps` feature with a spline basis, and adds low-order interactions before fitting a linear model.
