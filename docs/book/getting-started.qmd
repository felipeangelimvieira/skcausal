...
graph LR
  ...
  T["t: treatment"] --> F
  Y["y: outcome"] --> F
  F --> P["predict(requested treatments, X=None)"]
```
...
The built-in synthetic datasets are useful because they also expose `predict(...)`, which gives you the known target quantity for benchmarking.
