...
The grid can have any row count: `predict(t, X=None)` returns one average response per requested treatment row. If you omit `X`, X-aware estimators reuse the covariate sample stored during `fit`.
...
estimator.fit(X, t, y)
curve = estimator.predict_curve(grid)
```
