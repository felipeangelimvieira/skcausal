...
::: {.callout-important}
`evaluate_multiple_dataset_seeds` requires a dataset that subclasses `BaseKnownResponseDataset`, because the metrics compare predicted curves against the dataset's known ground-truth response. Both fully synthetic datasets (`BaseSyntheticDataset`) and the semi-synthetic ones qualify.
:::
...
1. Another dataset with known truth, such as a new subclass of `BaseSyntheticDataset` or one of the semi-synthetic datasets.
