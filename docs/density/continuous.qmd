...
```{mermaid}
flowchart TD
  Q1{"Does the downstream method\nrequire a normalized density\nor log-likelihood score?"}
  Q1 -->|Yes| A["SkproDensityEstimator\nReturns p(t|x)"]
  Q1 -->|No| Q2{"Is explicit density\nestimation difficult or\nunstable here?"}
  Q2 -->|Yes| B["PermutationWeighting\nReturns p(t|x)/p(t)"]
  Q2 -->|No| A
```
...
Check `estimator.get_tag("density_kind")` to confirm what a fitted estimator returns before plugging it into a weighting or evaluation step.
:::
...
|---|---|---|
| `treatment_transformation` | `None` | `"spline"` expands `t` with `SplineTransformer(n_knots=10, degree=2)` before classification — recommended for nonlinear treatment distributions |
| `fit_mode` | `"ensemble"` | `"ensemble"` fits up to `max_trials` classifiers and averages predicted probabilities; `"convergence"` keeps the first model whose observed-class probabilities show nontrivial variation |
| `max_trials` | `5` | Controls how many trial classifiers are fit in `"ensemble"` or `"convergence"` mode |
| `mean_type` | `"arithmetic"` | Switch to `"geometric"` for ensemble aggregation when ratio values span several orders of magnitude |
